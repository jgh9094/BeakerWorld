...
CXX_web := emcc
OFLAGS_web_all := -s "EXTRA_EXPORTED_RUNTIME_METHODS=['ccall', 'cwrap']" -s TOTAL_MEMORY=67108864 --js-library $(EMP_DIR)/web/library_emp.js -s EXPORTED_FUNCTIONS="['_main', '_empCppCallback', '_GetBlue']" -s DISABLE_EXCEPTION_CATCHING=1 -s NO_EXIT_RUNTIME=1#--embed-file configs
...
$(PROJECT):	source/BeakerWorld.h source/BeakerOrg.h source/native/$(PROJECT).cc
